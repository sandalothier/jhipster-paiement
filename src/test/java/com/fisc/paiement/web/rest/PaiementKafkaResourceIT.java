package com.fisc.paiement.web.rest;

import com.fisc.paiement.config.KafkaProperties;
...
class PaiementKafkaResourceIT {
    ...
    void setup() {
        ...
        PaiementKafkaResource kafkaResource = new PaiementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/paiement-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/paiement-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
